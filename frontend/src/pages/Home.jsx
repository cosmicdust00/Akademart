...
export default function Home() {
    ...
    const loadHomeData = async () => {
      ...
      try {
        ...
        if (Array.isArray(recomData)) {
          const otherRecoms = recomData.filter(p => p.seller_id !== user.user_id);
          setRecommendations(parseNeo4jPrice(otherRecoms));
        }

        const productsData = await api.products.getAll();
        if (Array.isArray(productsData)) {
          const availableAndNotMine = productsData.filter(
            p => p.status === "available" && p.seller_id !== user.user_id
          );

          setRecentProducts(parseNeo4jPrice(availableAndNotMine).slice(0, 4));

          const ownDeptListingsCount = productsData.filter(
            p => p.seller_jurusan === user?.prodi && p.seller_id !== user.user_id
          ).length;
          
          setStats({
            activeListings: availableAndNotMine.length,
  ...
  const handleAddToCart = async (product) => {
    if (product.seller_id === user?.user_id) {
      setToastMessage("Anda tidak bisa membeli produk sendiri.");
  ...
  // Menahan render UI utama sampai objek user dari Context tersedia
  if (!user) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-slate-100">
        <Loader className="w-10 h-10 text-violet-500 animate-spin mb-4" />
        <p className="text-sm text-slate-400">Menyiapkan sesi akun Anda...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-16">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 space-y-12">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-6"> {/* grid-cols-2 agar seimbang */}
